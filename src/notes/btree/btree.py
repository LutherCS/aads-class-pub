...
class BTreeNode:
    ...
    def __init__(self, degree=1, numberOfKeys=0, items=None, child=None, index=None):
    ...
    def __repr__(self):
        return (
            "BTreeNode("
            + str(len(self.items) // 2)
            + ","
            + str(self.numberOfKeys)
            + ","
            + repr(self.items)
            + ","
            + repr(self.child)
    ...
    def __str__(self):
        st = "The contents of the node with index " + str(self.index) + ":\n"
...
def main():
    ...
    for record in feedAttributeTable:
        ...
        offset = item.getValue()
        feedAttribTypeRecord = readRecord(
            feedAttribTypeTable, offset, attribTypeTableRecLength
        )
    ...
    print(
        "Done. The total time for the query without indexing was",
        milliseconds,
        "milliseconds.",
    )
